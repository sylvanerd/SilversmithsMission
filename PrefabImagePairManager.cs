...
namespace UnityEngine.XR.ARFoundation.Samples
{
    ...
    public class PrefabImagePairManager : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        void OnTrackedImagesChanged(ARTrackedImagesChangedEventArgs eventArgs)
        {
            ...
            //prefab on tracked image will gone except the nineth one
            //we want only the nineth one to stay there...
            foreach (var trackedImage in eventArgs.updated)
            {
                    if (trackedImage.trackingState != TrackingState.Tracking && trackedImage.referenceImage.name !="8Neptune")
                    {
                        if (m_Instantiated.TryGetValue(trackedImage.referenceImage.guid, out var gameObject))
                        {
                            Destroy(gameObject);
